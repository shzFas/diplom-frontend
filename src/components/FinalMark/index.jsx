...
function FinalMark({ studentId, predmetId, modal }) {
  ...
  useEffect(() => {
    axios
      .get(`${url}marks/final/${studentId}/${predmetId}/default`)
      .then((data) => {
        setMarkLesson(data.data);
      });
    axios
      .get(`${url}marks/final/${studentId}/${predmetId}/sor`)
      .then((data) => {
        setMarkSor(data.data);
      });
    axios
      .get(`${url}marks/final/${studentId}/${predmetId}/soch`)
      .then((data) => {
        setMarkSoch(data.data);
      });
  }, [location, studentId, predmetId, modal]);
  ...
  const soch = markSoch.map((data) => 
    data.markValue
  );

  const sochMax = markSoch.map((data) => 
    data.markMaxValue
  );

  const sor = markSor.map((data) => (
    data.markValue
  ))

  const sorMax = markSor.map((data) => (
    data.markMaxValue
  ))

  const fo = markLesson.map((data) => (
    data.markValue
  ))

  const finishResult = (((summArray(sor) + summArray(fo))/(summArray(sorMax) + fo.length * 10)) * 0.5 + (soch / sochMax) * 0.5)
